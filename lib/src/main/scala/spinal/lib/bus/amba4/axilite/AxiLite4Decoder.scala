...
/**
  ...
  * @param pendingMax maximum number of outstanding transactions
  */
case class AxiLite4ReadOnlyDecoder(axiConfig: AxiLite4Config,
                                   decodings: Seq[SizeMapping],
                                   pendingMax: Int = 7) extends Component {
  require(decodings.nonEmpty, "AXI4-Lite address decoding needs at least one decoding")
  assert(!SizeMapping.verifyOverlapping(decodings), "AXI4-Lite address decoding overlapping")
  ...
  if(decodingErrorPossible) {
    ...
    when(pendingError){
      io.input.readRsp.payload := errorSlave.io.axi.readRsp.payload
    }
...
/**
  * Route the write transactions of one AxiLite4 master toward several slaves, depending on the
  * address. Addresses which are not covered by `decodings` are answered by an
  * [[AxiLite4WriteOnlyErrorSlave]].
  *
  ...
  * @param pendingMax maximum number of outstanding transactions
  */
case class AxiLite4WriteOnlyDecoder(axiConfig: AxiLite4Config,
                                    decodings: Seq[SizeMapping],
                                    pendingMax: Int = 7) extends Component {
  require(decodings.nonEmpty, "AXI4-Lite address decoding needs at least one decoding")
  assert(!SizeMapping.verifyOverlapping(decodings), "AXI4-Lite address decoding overlapping")
  ...
  if(decodingErrorPossible) {
    when(pendingError){
      io.input.writeRsp.payload := errorSlave.io.axi.writeRsp.payload
    }
